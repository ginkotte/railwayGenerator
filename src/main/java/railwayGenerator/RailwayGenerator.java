...
//		System.out.println("Quantidade de curvas: " + curvas.size());
		...
		ArrayList<Element> cristas = generatePms(cristasArr);

		writeToFile(aclives, declives, planos, cristas, req);
...
//		}
	...
	public void writePontoDeMedida(Element pmLista, org.w3c.dom.Element listaDePontosDeMedida, Document doc,
			String id) {

		Element ponto = pmLista.getElementsByTag("listaDePontosDeMedida").first();
	...
	public ArrayList<Element> generateNewArray(ArrayList<Element> aclives, ArrayList<Element> declives,
		...
		for (int i = 0; i < req.getQtdAclives(); i++) {
			arr.add(aclives.get(random.nextInt(declives.size())));
		}
		...
		for (int i = 0; i < req.getQtdCristas(); i++) {
			arr.add(planos.get(random.nextInt(cristas.size())));
		}
